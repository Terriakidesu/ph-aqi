...
def main():
    ...
    # Pick a city and a model
    city = "Cebu City"
    model = "LSTM"
    
    # Prefer one-step rows if present; otherwise use the current recursive pipeline output.
    if "prediction_type" in df.columns:
        city_df = df[(df["city_name"] == city) & 
                     (df["model"] == model) & 
                     (df["prediction_type"] == "one_step")].sort_values("datetime")
        if city_df.empty:
    ...
    # Take a 48-hour window (if possible)
    start_idx = 0
    ...
    plt.step(plot_df["datetime"], plot_df["predicted_aqi"], where='post', label="Pred", linestyle='--', linewidth=2.0, color='blue')

    plt.title(f"48-Hour Forecast: {city}", fontsize=14, fontweight='bold')
    ...
    save_path = output_dir / f"{city.lower().replace(' ', '_')}_forecast_48h.png"
    plt.savefig(save_path, dpi=300, bbox_inches="tight")
    print(f"Saved forecast sample plot to: {save_path}")
